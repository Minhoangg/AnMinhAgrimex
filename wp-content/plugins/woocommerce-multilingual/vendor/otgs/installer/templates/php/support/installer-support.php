<div class="wrap">
    ...
    <div class="otgs-installer-support-test-connection">
        <h2><?php echo wp_kses_post( $model->strings->tester->title ); ?> <a
                    class="button check-again"><?php echo wp_kses_post( $model->strings->tester->button_label ); ?></a>
        </h2>

        <ul><?php foreach ( $model->tester->endpoints as $endpoint ) { ?>
                <li class="endpoint" data-repository="<?php echo esc_attr( $endpoint->repository ); ?>"
                    data-type="<?php echo esc_attr( $endpoint->type ); ?>"><span
                            class="dashicons dashicons-yes status"></span> <?php echo esc_attr( $endpoint->description ); ?>
                </li>
    ...
    <div class="otgs-installer-support-test-connection">
        ...
        <ul>
					<?php foreach ( $model->requirements as $requirement ) {
						...
						?>
              <li>
                  <span class="dashicons  <?php echo esc_attr( $icon_class ); ?> status"></span> <?php echo wp_kses_post( $requirement->name ) ?>
              </li>
					<?php } ?>
        </ul>
