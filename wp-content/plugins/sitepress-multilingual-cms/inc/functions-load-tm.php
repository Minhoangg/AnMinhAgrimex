...
if ( ! Plugins::isTMActive() && ( ! wpml_is_setup_complete() || false !== SetupOptions::isTMAllowed() ) ) {
	...
	function wpml_tm_create_ATE_job_creation_model( $job_id ) {
		...
		if ( $previousStatus->map( Obj::prop( 'status' ) )->getOrElse( null ) === (string) ICL_TM_ATE_CANCELLED ) {
			...
			$job->existing_ate_id = make( JobRecords::class )->get_ate_job_id( $job_id );
		} else {
			...
			$job->notify_url     = PublicReceive::get_receive_ate_job_url( $job_id );
