...
use WPML\AbsoluteLinks\BlockProtector;
use WPML\FP\Lst;
use WPML\FP\Str;
use WPML\ST\SlugTranslation\Hooks\HooksFactory;
...
class AbsoluteLinks {
	...
	public function _process_generic_text( $source_text, &$alp_broken_links ) {
		...
		$block_protector = new BlockProtector();
		...
		foreach ( $this->active_languages as $test_language ) {
			...
			if ( $int1 || $int2 ) {
				...
				foreach ( $alp_matches[4] as $k => $dir_path ) {
					...
					[ $lang, $dir_path ] = $this->extract_lang_from_path( $sitepress_settings, $default_language, $dir_path );
					...
					if ( isset( $permalink_query_vars['pagename'] ) ) {
						...
						[ $post_type, $post_name ] = $wpdb->get_row(
	...
	public function all_rewrite_rules( $rewrite ) {
		...
		foreach ( $active_languages as $next_language ) {
			...
			$translated_rules = ( new HooksFactory() )->create()->filter( $final_rules );
