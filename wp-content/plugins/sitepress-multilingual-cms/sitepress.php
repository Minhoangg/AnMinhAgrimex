...
use WPML\AdminLanguageSwitcher\DisableWpLanguageSwitcher;
use WPML\AdminMenu\Redirect;
use WPML\BrowserLanguageRedirect\Dialog;
use WPML\CLI\Core\BootStrap;
use WPML\Container\Config;
use WPML\Core\BackgroundTask;
use WPML\Core\Menu\Translate;
use WPML\FullSiteEditing\BlockTemplates;
use WPML\Installer\AddSiteUrl;
use WPML\Installer\DisableRegisterNow;
use WPML\LanguageSwitcher\AjaxNavigation\Hooks;
use WPML\Media\Duplication\HooksFactory;
use WPML\Media\FrontendHooks;
use WPML\Media\Loader;
use WPML\Media\Translate\LanguagesUpdated;
use WPML\Options\Reset;
use WPML\Requirements\WordPress;
use WPML\REST\XMLConfig\Custom\Factory;
use WPML\Roles;
use WPML\Settings\UI;
use WPML\Setup\DisableNotices;
use WPML\TaxonomyTermTranslation\AutoSync;
use WPML\UrlHandling\WPLoginUrlConverterFactory;
...
if ( ! WordPress::checkMinimumRequiredVersion() ) {
	return;
}
...
if ( $sitepress->is_setup_complete() ) {
	$actions = [
		...
		Hooks::class,
		Dialog::class,
		WPLoginUrlConverterFactory::class,
		Roles::class,
		...
		Translate::class,
		AutoSync::class,
		BlockTemplates::class,
		DisableWpLanguageSwitcher::class,
		AdminLanguageSwitcher::class,
	];
	...
	$action_filter_loader->load( [
		DisableNotices::class,
		DisableRegisterNow::class,
	] );
}
...
function wpml_init_cli() {
	$wpml_cli_bootstrap = new BootStrap();
