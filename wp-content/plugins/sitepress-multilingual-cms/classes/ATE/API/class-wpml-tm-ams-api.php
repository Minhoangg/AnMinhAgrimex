...
class WPML_TM_AMS_API {
	...
	private function handleErrorResponse($logErrorResponse, $getErrors) {
		return curryN( 3, function ( $shouldHandleError, $errorHandler, $data ) use ( $logErrorResponse, $getErrors ) {
			[ $response, $uuid ] = $data;
	...
	private function ping_healthy_wpml_endpoint() {
		...
		return ! is_wp_error( $response ) && (int) Obj::path( [ 'response', 'code' ], $response ) === 200;
	}
