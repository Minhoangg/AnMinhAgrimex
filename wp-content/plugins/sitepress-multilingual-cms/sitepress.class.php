...
class SitePress extends WPML_WPDB_User implements
	...
	function verify_settings() {
		...
		[ $this->settings, $update_settings ] = $verify_settings->verify( $this->settings );
	...
	function post_edit_language_options() {
		...
		if ( current_user_can( 'manage_options' ) && OptionManager::getOr( false, 'core', 'show_cf_meta_box' ) ) {
	...
	public function get_ls_languages( $template_args = array() ) {
		...
		[ $translations, $wp_query ] = $languages_helper->get_ls_translations(
			$wp_query,
			$_wp_query_back,
			$this->wp_query
		);
		...
		foreach ( $w_active_languages as $k => $lang ) {
			...
			if ( is_singular()
				...
				[ $lang, $skip_lang ] = $languages_helper->add_tax_url_to_ls_lang(
					$lang,
					$translations,
					$icl_lso_link_empty,
					$skip_lang,
				...
				[ $lang, $skip_lang ] = $languages_helper->add_author_url_to_ls_lang(
					$lang,
					$default_language,
					$authordata,
					$icl_lso_link_empty,
				...
				[ $lang, $skip_lang ] = $languages_helper->add_date_or_cpt_url_to_ls_lang(
					$lang,
					$default_language,
					$this->wp_query,
					$icl_lso_link_empty,
