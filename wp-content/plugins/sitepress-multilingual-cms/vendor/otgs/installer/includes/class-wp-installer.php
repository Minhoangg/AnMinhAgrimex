...
class WP_Installer{
    ...
    protected $_plugins_renew_warnings = [];

    private $admin_messages = [];

    private $ajax_messages = [];
    ...
    public function __construct(){
        ...
        add_action('admin_init', [$this, 'load_embedded_plugins'], 0);

        add_action('admin_menu', [$this, 'menu_setup']);
        add_action('network_admin_menu', [$this, 'menu_setup']);
        add_filter('plugins_api', [$this, 'custom_plugins_api_call'], 10, 3);
        ...
        add_action('init', [$this, 'load_locale']);
    }
    ...
    public function init(){
        ...
        if (is_multisite() || !$this->is_installer_running_on_otgs_plugin() || ($this->is_commercial_page() && $this->is_installer_running_on_otgs_plugin())){
            wp_enqueue_script('installer-admin', $this->res_url() . '/res/js/admin.js', ['jquery'],
                $this->version());
            wp_enqueue_style('installer-admin', $this->res_url() . '/res/css/admin.css', [],
                $this->version());
            wp_enqueue_style('installer-admin-notices',
                $this->res_url() . '/res/css/admin-notices.css', [], $this->version());
        }

        if ($this->is_commercial_page()){
            wp_enqueue_script('expired-notice', $this->res_url() . '/dist/js/expired-notice/app.js',
                [], $this->version());
        }

        wp_enqueue_script('installer-dismiss-nag', $this->res_url() . '/res/js/dismiss-nag.js',
            ['jquery'], $this->version(), TRUE);
        wp_enqueue_script('install-recommended_plugin',
            $this->res_url() . '/res/js/install_recommended_plugin.js', ['jquery'],
            $this->version(), TRUE);
        ...
        if ($pagenow == 'plugins.php'){
            add_action('admin_notices', [$this, 'setup_plugins_page_notices']);
            add_action('admin_notices', [$this, 'setup_plugins_renew_warnings'], 10);
            add_action('admin_notices', [$this, 'queue_plugins_renew_warnings'], 20);

            add_action('admin_init', [$this, 'setup_plugins_action_links']);

            wp_enqueue_script('installer-plugins', $this->res_url() . '/res/js/plugins.js',
                ['jquery'], $this->version());
            wp_enqueue_style('installer-admin-notices',
                $this->res_url() . '/res/css/admin-notices.css', [], $this->version());
        }
        ...
        if ($this->is_repositories_page()){
            add_action('admin_init', [$this, 'validate_repository_subscription']);
        }

        if (defined('DOING_AJAX')){
            add_action('wp_ajax_installer_download_plugin',
                [$this, 'download_plugin_ajax_handler']);
            add_action('wp_ajax_installer_activate_plugin', [$this, 'activate_plugin']);
        }

        if ($pagenow === 'update.php'){
            if (isset($_GET['action']) && $_GET['action'] === 'update-selected'){
                add_action('admin_head',
                    [$this, 'plugin_upgrade_custom_errors']);         //iframe/bulk
            }else{
                add_action('all_admin_notices',
                    [$this, 'plugin_upgrade_custom_errors']);  //regular/singular
            }
        }

        // WP 4.2
        if (defined('DOING_AJAX')){
            add_action('wp_ajax_update-plugin', [
                $this,
    ...
    private function is_installer_running_on_otgs_plugin(){
        return (defined('ICL_PLUGIN_PATH') && FALSE !== strpos($this->plugin_path(),
                    realpath(ICL_PLUGIN_PATH)))
               || (defined('TYPES_ABSPATH') && FALSE !== strpos($this->plugin_path(),
                    realpath(TYPES_ABSPATH)))
               || (defined('WCML_PLUGIN_PATH') && FALSE !== strpos($this->plugin_path(),
                    realpath(WCML_PLUGIN_PATH)));
    }
    ...
    public function log($message){
        if (file_exists(ABSPATH . 'wp-admin/includes/file.php')){
            ...
            if (defined('WPML_INSTALLER_LOGGING') && WPML_INSTALLER_LOGGING){
                $wp_filesystem->put_contents($this->plugin_path() . '/installer.log',
    ...
    public function register_admin_message($text, $type = 'updated'){
        $this->admin_messages[] = ['text' => $text, 'type' => $type];
        if (defined('DOING_AJAX')){
    ...
    public function menu_setup(){
        ...
        if (is_multisite() && !is_network_admin()){
            ...
            if ($this->config['plugins_install_tab'] && is_admin() && $pagenow === 'plugin-install.php'){
                ...
                add_filter('install_plugins_tabs', [$this, 'add_install_plugins_tab']);
                add_action('install_plugins_commercial', [$this, 'show_products']);
            }
        }
    }
                        ...
                        foreach ($package['products'] as $product){
                            if (array_key_exists('plugins', $product)){
                                foreach ($product['plugins'] as $plugin_slug){
                                    ...
                                    if (!$this->isPluginAvailableInRepositoryDownloads($repository_id,
                                        $plugin_slug)){
                                        continue;
                                    }
                                    ...
                                    if (!isset($repositories_plugins[$repository_id][$download['slug']])){
                                        $repositories_plugins[$repository_id][$download['slug']] = [
                                            'name'       => $download['name'],
                                            'registered' => $this->plugin_is_registered($repository_id,
                                                $download['slug']) ? 1 : 0
                                        ];
                                    }
                        ...
                        foreach ($r_plugins as $slug => $r_plugin){
                            ...
                            if ($wp_plugin_slug === $slug || $r_plugin['name'] === $plugin['Name'] || $r_plugin['name'] === $plugin['Title']){ //match order: slug, name, title
                                ...
                                if ($r_plugin['registered']){
                                    ...
                                    remove_filter('plugin_action_links_' . $plugin_id, [
                                        $this,
                                        'plugins_action_links_not_registered'
                                    ]);

                                    add_filter('plugin_action_links_' . $plugin_id, [
                                        $this,
                                        'plugins_action_links_registered'
                                    ]);
                                }else{

                                    if ($this->plugin_is_registered($plugin_obj->get_external_repo(),
                                            $slug) || $this->plugin_is_registered('wpml', $slug)){
                                        continue;
                                    }
                                    ...
                                    if ($this->should_display_types_upgrade_link($r_plugin['name'],
                                        $plugin['Version'])){
                                        add_filter('plugin_action_links_' . $plugin_id,
                                            [$this, 'types_upgrade_link']);
                                    }
                                }
                            }
    ...
    public function types_upgrade_link($links){
        $links[] = '<a style="color: #55AA55" target="_blank" href="' . esc_url('https://toolset.com/buy/?utm_source=typesplugin&utm_campaign=moving-types-to-toolset&utm_medium=plugins-page&utm_term=upgrade-link') . '">' . __('Upgrade',
                'installer') . '</a>';
    ...
    public function get_embedded_at(){
        $embedded_at = str_replace([get_template_directory_uri(), plugins_url()], '',
            $this->plugin_url());
                        ...
                        if (!empty($response['error'])){
                            ...
                            $this->admin_messages[] = [
                                ...
                                'text' => sprintf(__('You are using an invalid site key defined as the constant %s (most likely in wp-config.php).
                                    ...
                                    'installer'),
                                    'OTGS_INSTALLER_SITE_KEY_' . strtoupper($repository_id))
                            ];
    ...
    private function _old_products_format_backwards_compatibility($settings){
        ...
        if (version_compare($this->version(), '1.8',
                '<') && !empty($settings['repositories']) && empty($this->_old_products_format_backwards_compatibility)){
    ...
    public function get_installer_site_url($repository_id = FALSE){
        ...
        $filtered_site_url = filter_var(apply_filters('otgs_installer_site_url', $site_url),
            FILTER_SANITIZE_URL);
        ...
        if ($config_file){
            ...
            if ($repos){
                foreach ($repos as $repo){
                    ...
                    // excludes rule;
                    if (isset($this->config['repositories_exclude']) && in_array($id,
                            $this->config['repositories_exclude'])){
                        continue;
                    }
    ...
    public function refresh_subscriptions_data(){
        ...
        foreach ($this->repositories as $repository_id => $data){
            ...
            try{
                [$subscription_data, $site_key_data] = $this->fetch_subscription_data(
                    $repository_id,
                    $site_key,
    ...
    public function refresh_repositories_data($bypass_bucket = FALSE){
        ...
        if (defined('OTGS_DISABLE_AUTO_UPDATES') && OTGS_DISABLE_AUTO_UPDATES && empty($_GET['force-check']) || $this->repositories_already_refreshed){
            ...
            if (empty($this->settings['repositories']) && $this->is_repositories_page()){
                ...
                $error = sprintf(__("Installer cannot display the products information because the automatic updating for %s was explicitly disabled with the configuration below (usually in wp-config.php):",
                    'installer'), strtoupper(join(', ', $repository_names)));
                $error .= '<br /><br /><code>define("OTGS_DISABLE_AUTO_UPDATES", true);</code><br /><br />';
                $error .= sprintf(__("In order to see the products information, please run the %smanual updates check%s to initialize the products list or (temporarily) remove the above code.",
                    'installer'), '<a href="' . admin_url('update-core.php') . '">', '</a>');
        ...
        foreach ($this->repositories as $repository_id => $data){
            ...
            if (is_wp_error($response)){
                ...
                $this->register_admin_message($error, 'error');
                $this->store_log($products_url, NULL,
                    OTGS_Installer_Logger_Storage::COMPONENT_REPOSITORIES, $error);
            ...
            if ($response && isset($response['response']['code']) && $response['response']['code'] == 200){
                try{
                    $products = $products_parser->get_products_from_response($products_url,
                    ...
                    $error = __("Information about new versions is invalid. It may be a temporary communication problem, please check for updates again.",
                        'installer');
                    $this->register_admin_message($error, 'error');
                }
            }else{
    ...
    public function show_products($args = []){
        if (!current_user_can('install_plugins')){
            wp_die(__('Sorry, you are not allowed to manage Installer for this site.',
                'installer'));
    ...
    /**
     * @param string $repositoryId
     * @param array $downloads List of plugins data that are available for subscription type and
     *     used on commercial tab.
     *
     * @return array
     */
    ...
    private function _render_product_packages(
        $packages,
        $subscription_type,
        $expired,
        $upgrade_options,
            ...
            foreach ($package['products'] as $product){
                ...
                if (empty($subscription_type) || $expired){
                    ...
                    if (!empty($product['price_disc'])){
                        $p['label'] = $product['call2action'] . ' - ' . sprintf('$%s %s$%d%s (USD)',
                                $product['price_disc'], '&nbsp;&nbsp;<del>', $product['price'],
                                '</del>');
                    }else{
                        $p['label'] = $product['call2action'] . ' - ' . sprintf('$%d (USD)',
                                $product['price']);
                    }
                    $row['products'][] = $p;
                    ...
                    if ($product['renewals']){
                        foreach ($product['renewals'] as $renewal){
                            $p['url']   = $this->append_parameters_to_buy_url($renewal['url'],
                                $repository_id);
                            $p['label'] = $renewal['call2action'] . ' - ' . sprintf('$%d (USD)',
                                    $renewal['price']);
                        }
                ...
                if (!empty($upgrade_options[$product['subscription_type']])){
                    ...
                    foreach ($upgrade_options[$product['subscription_type']] as $stype => $upgrade){
                        ...
                        if (!empty($upgrade['price_disc'])){
                            $p['label'] = $upgrade['call2action'] . ' - ' . sprintf('$%s %s$%d%s (USD)',
                                    $upgrade['price_disc'], '&nbsp;&nbsp;<del>', $upgrade['price'],
                                    '</del>');
                        }else{
                            $p['label'] = $upgrade['call2action'] . ' - ' . sprintf('$%d (USD)',
                                    $upgrade['price']);
                        }
                        $row['products'][] = $p;
    ...
    public function append_parameters_to_buy_url($url, $repository_id, $args = []){

        $url = add_query_arg(['icl_site_url' => $this->get_installer_site_url($repository_id)],
            $url);
        ...
        if ($affiliate_id && $affiliate_key){
            $url = add_query_arg(['aid' => $affiliate_id, 'affiliate_key' => $affiliate_key], $url);
        }
    ...
    public function save_site_key($args = []){
        ...
        if ($repository_id && $nonce && wp_verify_nonce($nonce, 'save_site_key_' . $repository_id)){
            ...
            try{
                [$subscription_data, $site_key_data] = $this->fetch_subscription_data($repository_id,
                    $site_key, self::SITE_KEY_VALIDATION_SOURCE_REGISTRATION);

                if ($subscription_data){
                    $this->settings['repositories'][$repository_id]['subscription'] = [
                        'key'           => $site_key,
                ...
                $error = $e->getMessage();
                if (preg_match('#Could not resolve host: (.*)#', $error,
                        $matches) || preg_match('#Couldn\'t resolve host \'(.*)\'#', $error,
                        $matches)){
                    $error = sprintf(__("%s cannot access %s to register. Try again to see if it's a temporary problem. If the problem continues, make sure that this site has access to the Internet.",
                        'installer'),
    ...
    public function validate_repository_subscription(){
        ...
        if ($repository_id){
            ...
            if ($site_key){
                try{
                    [$subscription_data, $site_key_data] = $this->fetch_subscription_data($repository_id,
                        $site_key, self::SITE_KEY_VALIDATION_SOURCE_REVALIDATION);
                }catch (Exception $e){
    ...
    public function fetch_subscription_data(
        $repository_id,
        $site_key,
        $source = self::SITE_KEY_VALIDATION_SOURCE_OTHER){
            ...
            foreach ($package['products'] as $product_id => $product){
                ...
                foreach ($product['plugins'] as $plugin_slug){
                    ...
                    if (empty($versions[$download['slug']])){
                        $v = $this->get_plugin_installed_version($download['name'],
                            $download['slug']);
                        if ($v){
        ...
        if (!empty($data['subscriptions_meta'])){
            if (isset($data['subscriptions_meta']['expiration'])){
                ...
                if (!empty($data['subscriptions_meta']['expiration'][$subscription_id])){
                    ...
                    //defaults
                    $days    = 30;
                    $message = __('You will have to renew your subscription in order to continue getting the updates and support.',
                        'installer');
                ...
                if (!empty($this->settings['repositories'][$repository_id]['subscription'])){
                    ...
                    if ($subscription['data']->subscription_type == $subscription_id && !empty($subscription['data']->expires)){
                        ...
                        if (strtotime($subscription['data']->expires) < strtotime(sprintf("+%d day",
                                $days))){

                            $days_to_expiration = ceil((strtotime($subscription['data']->expires) - time()) / 86400);

                            echo '<div><p class="installer-warn-box">' .
                                 sprintf(_n('Your subscription expires in %d day.',
                                     'Your subscription expires in %d days.', $days_to_expiration,
                                     'installer'), $days_to_expiration) .
                                 '<br />' . $message .
                                 '</p></div>';
            ...
            if ($this->repository_has_valid_subscription($repository_id)){
                ...
                if (!$never_expires){
                    if (isset($this->settings['repositories'][$repository_id]['data']['subscriptions_meta']['expiration'][$subscription_type])){
                        ...
                        //defaults
                        $days_warning   = 30;
                        $custom_message = __('You will have to renew your subscription in order to continue getting the updates and support.',
                            'installer');
                    }
                    ...
                    if (strtotime($expires) < strtotime(sprintf('+%d day', $days_warning))){
                        ...
                        $message                                         = sprintf(_n('Your subscription expires in %d day.',
                            'Your subscription expires in %d days.', $days_to_expiration,
                            'installer'), $days_to_expiration);
    ...
    public function queue_plugins_renew_warnings(){
        ...
        if (!empty($this->_plugins_renew_warnings)){
            ...
            foreach ($this->_plugins_renew_warnings as $plugin_id => $message){
                ...
                add_action("after_plugin_row_" . $plugin_id, [$this, 'plugins_renew_warning'], 10,
                    3);
            }
    ...
    public function plugin_is_installed($name, $slug, $version = NULL){
        ...
        if (!$is && $name == 'Types'){
            return $this->plugin_is_installed('Types - Complete Solution for Custom Fields and Types',
                $slug, $version);
        }
    ...
    public function download_plugin_ajax_handler(){
        ...
        try{
            [$subscription_data, $site_key_data] = $this->fetch_subscription_data($data['repository_id'],
                $site_key, self::SITE_KEY_VALIDATION_SOURCE_DOWNLOAD_REPORT);
        }catch (Exception $e){
            $connection_error  = $e->getMessage();
            $subscription_data = FALSE;

            $this->store_log($data_url, $data_url_args,
                OTGS_Installer_Logger_Storage::COMPONENT_SUBSCRIPTION, $connection_error);
        }
        ...
        if ($subscription_data && !is_wp_error($subscription_data) && $this->repository_has_valid_subscription($data['repository_id'])){
            if (wp_verify_nonce($data['nonce'], 'install_plugin_' . $data['url'])){
                ...
                foreach ($plugins as $id => $plugin){
                    ...
                    $is_embedded    = $this->plugin_is_embedded_version(preg_replace('/ Embedded$/',
                        '', $plugin['Name']), preg_replace('/-embedded$/', '', $wp_plugin_slug));

                    if ($wp_plugin_slug == $data['slug'] || $is_embedded && preg_replace('/-embedded$/',
                            '', $wp_plugin_slug) == $data['slug']){
                        $plugin_id = $id;
                        break;
                    }
                }
                ...
                if ($plugin_id && !$is_embedded && $this->is_plugin_out_of_date($plugin_id)){ //upgrade
                    ...
                    if (!$ret && !empty($upgrader->skin->installer_error)){
                        ...
                        $plugin_version = $this->get_plugin_repository_version($data['repository_id'],
                            $data['slug']);
                    }
                ...
                if ($ret){
                    foreach ($plugins as $id => $plugin){
                        ...
                        if ($wp_plugin_slug == $data['slug']){
                            ...
                            $this->include_auto_upgrade_during_install($data['repository_id'],
                                $plugin_id);
                            break;
                        }
                    }
                ...
                if (WP_Installer_Channels()->get_channel($data['repository_id']) !== WP_Installer_Channels::CHANNEL_PRODUCTION){
                    ...
                    $non_stable = WP_Installer_Channels()->get_download_source_channel($plugin_version,
                        $data['repository_id'], $download['slug'], 'plugins');
                }
            ...
            $ret     = FALSE;
            $message = sprintf(__('Connection failed! Please refresh the page and try again. (%s)',
                'installer'), '<i>' . $connection_error . '</i>');
        }else{ //subscription not valid
            $ret     = FALSE;
            $message = __('Your subscription appears to no longer be valid. Please try to register again using a valid site key.',
                'installer');
        }

        if ($message){
            $this->store_log($data_url, $data_url_args,
                OTGS_Installer_Logger_Storage::COMPONENT_DOWNLOAD, $message);
        }
    ...
    public function activate_plugin(){
        ...
        if (isset($_POST['nonce']) && $plugin_id && wp_verify_nonce($_POST['nonce'],
                'activate_' . $plugin_id)){
                ...
                foreach ($repository['data']['packages'] as $package){
                    ...
                    foreach ($package['products'] as $product){
                        ...
                        foreach ($product['plugins'] as $plugin_slug){
                            ...
                            $plugin_finder = $this->get_plugin_finder();
                            $plugin_found  = $plugin_finder->get_plugin($plugin_slug,
                                $repository_id);
                            ...
                            if ($external_repo && $this->plugin_is_registered($external_repo,
                                    $plugin_slug)){
                                continue;
                            }
                            ...
                            if ($download['slug'] == $slug || $download['name'] == $name){
                                if (in_array($name, ['Toolset Types', 'WPML Multilingual CMS'],
                                    TRUE)){
                                    $display_setting_notice = TRUE;
                                }

                                if (!$site_key || !$this->plugin_is_registered($repository_id,
                                        $download['slug'])){
                                    $display_setting_notice = FALSE;
                                ...
                                if ($this->plugin_is_registered($repository_id,
                                        $download['slug']) && $this->repository_has_legacy_free_subscription($repository_id)){
                                    $display_subscription_notice = [
                                        'type'    => 'legacy_free',
                                        'repo'    => $repository_id,
                                        'product' => $repository['data']['product-name']
    ...
    public function get_matching_cp($repository, $args = []){
        $match = FALSE;
    ...
    public function set_filtered_prices($args = []){
            ...
            foreach ($repository['data']['packages'] as $package_id => $package){
                ...
                foreach ($package['products'] as $product_id => $product){
                    ...
                    if ($match['dtp'] == '%'){
                        ...
                        $fprice = $fprice != round($fprice) ? sprintf('%.2f',
                            $fprice) : round($fprice, 0);
                    }elseif ($match['dtp'] == '-'){
                    ...
                    if ($fprice){
                        ...
                        $url_glue                                                                                                        = FALSE !== strpos($this->settings['repositories'][$repository_id]['data']['packages'][$package_id]['products'][$product_id]['url'],
                            '?') ? '&' : '?';
                        $cpndata                                                                                                         = base64_encode(json_encode([
    ...
    public function set_hierarchy_and_order(){
        ...
        if (!empty($this->settings['repositories'])){
            foreach ($this->settings['repositories'] as $repository_id => $repository){
                ...
                // order parents
                usort($ordered_packages, [$this, 'compare_package_order']);
                //order sub-packages
                foreach ($ordered_packages as $package_id => $package){
                    if (!empty($package['sub-packages'])){
                        usort($ordered_packages[$package_id]['sub-packages'],
                            [$this, 'compare_package_order']);
                    }
                }
    ...
    private function get_plugin_finder(){
        if (!$this->plugin_finder){
            $this->plugin_finder = new OTGS_Installer_Plugin_Finder(new OTGS_Installer_Plugin_Factory(),
                $this->settings['repositories']);
        }
        ...
        if (isset($_REQUEST['action'])){
                    ...
                    foreach ($plugins as $k => $plugin){
                        ...
                        if ($plugin_repository){
                            ...
                            if (empty($sub_cache[$plugin_repository])){
                                ...
                                if (!$site_key){
                                    [$plugin_repository, $site_key] = $this->match_product_in_external_repository($plugin_repository,
                                        $wp_plugin_slug);
                                }

                                if ($site_key){
                                    try{
                                        [$subscription_data, $site_key_data] = $this->fetch_subscription_data($plugin_repository,
                                            $site_key,
                            ...
                            if (!$site_key && (!empty($free_on_wporg) || $this->should_fallback_under_wp_org_repo($download,
                                        $site_key))){ // allow the download from wp.org
                                continue;
                            }
                            ...
                            if (empty($site_key) || empty($subscription_data)){
                                ...
                                $error_message = sprintf(__("%s cannot update because your site's registration is not valid. Please %sregister %s%s again for this site first.",
                                    'installer'),
                                    '<strong>' . $plugin_name . '</strong>',
                                    '<a target="_top" href="' . $this->menu_url() . '&validate_repository=' . $plugin_repository .
                                    '#repository-' . $plugin_repository . '">', $product_name,
                                    '</a>');
            ...
            if ('upgrade-plugin' == $action || 'update-plugin' == $action){
                ...
                if ($plugin_repository){
                    ...
                    $not_on_wporg    = empty($free_on_wporg) && !$this->should_fallback_under_wp_org_repo($download,
                            $site_key);
                    ...
                    if ($no_subscription && $not_on_wporg){
                        ...
                        $error_message = sprintf(__("%s cannot update because your site's registration is not valid. Please %sregister %s%s again for this site first.",
                            'installer'),
                            '<strong>' . $plugin_name . '</strong>',
                            '<a href="' . $this->menu_url() . '&validate_repository=' . $plugin_repository .
                        ...
                        if (defined('DOING_AJAX')){ //WP 4.2
                            ...
                            echo '<a href="' . admin_url('plugins.php') . '">' . __('Return to the plugins page',
                                    'installer') . '</a>';
                            echo '</div>';
