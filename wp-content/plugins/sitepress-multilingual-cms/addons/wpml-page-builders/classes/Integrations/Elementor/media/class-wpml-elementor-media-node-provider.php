...
use WPML\PB\Elementor\Media\Modules\AllNodes;
use WPML\PB\Elementor\Media\Modules\Gallery;
use WPML\PB\Elementor\Media\Modules\Hotspot;
use WPML\PB\Elementor\Media\Modules\Video;
use WPML\PB\Elementor\Media\Modules\VideoPlaylist;
	...
	public function get( $type ) {
		if ( ! array_key_exists( $type, $this->nodes ) ) {
			switch ( $type ) {
				...
				case 'all_nodes':
					$node = new AllNodes( $this->media_translate );
					break;

				case 'video':
					$node = new Video( $this->media_translate );
					break;

				case 'video-playlist':
					$node = new VideoPlaylist( $this->media_translate );
					break;

				case 'hotspot':
					$node = new Hotspot( $this->media_translate );
					break;
